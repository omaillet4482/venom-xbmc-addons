...
SITE_IDENTIFIER = 'elitegol_b'
SITE_NAME = 'Elitegol BIS'
...
SPORT_GENRES = ('/', 'showMovies','ch24.php')  # FOOT
SPORT_LIVE = ('/', 'showMovies', 'json.php?ldc')
SPORT_TV = ('/', 'showMovies','ch24.php')
...
def showMovies():
    ...
    if sHtmlContent:
        jsonData = json.loads(sHtmlContent)
        oOutputParameterHandler = cOutputParameterHandler()
        #web_pdb.set_trace()
        for aEntry in jsonData:
            sTitle = ' %s-%s ' % (aEntry['home'], aEntry['away'])
            sTime = aEntry['time']
            sDate = datetime.fromtimestamp(int(sTime) / 1000)
            sUrl2 = aEntry['streams']
            sLang = ''
            sThumb = ''
            sType = aEntry['type']
            ...
            oGui.addDir(SITE_IDENTIFIER, 'showStreams', sDisplayTitle, "sport.png", oOutputParameterHandler)
...
def showStreams():
    ...
    sThumb = ''  
    #web_pdb.set_trace()
    jsonData = json.loads(sUrl)
    for chaine in jsonData:
       lang = chaine['lang']
